"""
MiniPascal v3 — Analizador sintáctico.
...
Gramática de v3 (lo nuevo de esta semana, marcado con «NUEVO»):
    ...
    instruccion   -> WRITELN PARIZQ expresion PARDER PUNTOCOMA
                   | bloque PUNTOCOMA
                   | VAR IDENTIFICADOR DOSPUNTOS TIPO PUNTOCOMA
                   | VAR IDENTIFICADOR DOSPUNTOS TIPO ASIGNACION expresion PUNTOCOMA
                   | CONST IDENTIFICADOR DOSPUNTOS TIPO ASIGNACION expresion PUNTOCOMA
                   | IDENTIFICADOR ASIGNACION expresion PUNTOCOMA
                   | IF expresion THEN bloque PUNTOCOMA                            «NUEVO»
                   | IF expresion THEN bloque ELSE bloque PUNTOCOMA                «NUEVO»
                   | WHILE expresion DO bloque PUNTOCOMA                           «NUEVO»
                   | IDENTIFICADOR DOSPUNTOS WHILE expresion DO bloque PUNTOCOMA   «NUEVO»
                   | REPEAT instrucciones UNTIL expresion PUNTOCOMA                «NUEVO»
                   | CASE expresion OF ramas END PUNTOCOMA                         «NUEVO»
                   | CASE expresion OF ramas ELSE bloque END PUNTOCOMA             «NUEVO»
                   | BREAK [IDENTIFICADOR] PUNTOCOMA                               «NUEVO»
                   | CONTINUE [IDENTIFICADOR] PUNTOCOMA                            «NUEVO»
    ramas         -> ramas rama | rama                                            «NUEVO»
    rama          -> ENTERO DOSPUNTOS bloque PUNTOCOMA                             «NUEVO»
    expresion     -> expresion (MAS|MENOS|POR|ENTRE) expresion
                   | expresion (IGUAL|DISTINTO|MENOR|MAYOR|MENORIGUAL|MAYORIGUAL) expresion «NUEVO»
                   | MENOS expresion
...
#
# IFX/ELSE: el mismo truco que en micro/02_conflictos.py, para el
# "dangling else" de `if`. Aquí NO resultó hacer falta en la práctica
# (revisen el README para el porqué), pero se deja igual, explícito, en
# vez de confiar en que el comportamiento por defecto de PLY siga siendo
# el correcto si la gramática cambia más adelante.
#
# Las comparaciones (==, !=, <, etc.) van con precedencia MENOR que la
# aritmética, para que `a + 1 < b * 2` se lea como `(a + 1) < (b * 2)` y
# no intente comparar antes de sumar/multiplicar. Y son 'nonassoc' porque
# encadenar `a < b < c` no tiene un significado razonable en MiniPascal.
# ---------------------------------------------------------------------------
precedence = (
...
# if / else
#
# `%prec IFX` en la primera regla: ver la explicación en el bloque de
# `precedence`, arriba, y en el README de esta sesión.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# repeat...until — reutiliza `instrucciones` directamente, sin pasar por
# `bloque`: no hay BEGIN/END que reducir, `until` es su propio cierre.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# case...of, con ramas de enteros y un else opcional
# ---------------------------------------------------------------------------
...
def p_instruccion_case_con_else(p):
    # El `bloque` del `else` termina en `;`, igual que el de cada `rama`
    # (`ENTERO DOSPUNTOS bloque PUNTOCOMA`) — a propósito, para que las
    # ramas se vean todas iguales y no haya que acordarse de una excepción.
...
def p_ramas_una(p):
    'ramas : rama'
    # Sin producción vacía a propósito: un `case` con cero ramas no tiene
    # sentido, así que ni lo permitimos gramaticalmente.
    p[0] = [p[1]]
...
# ---------------------------------------------------------------------------
# break / continue, con y sin etiqueta
# ---------------------------------------------------------------------------
