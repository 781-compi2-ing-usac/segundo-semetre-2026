"""
MiniPascal v3 — Analizador léxico.

Novedades de v3 frente a v2:
  - Palabras reservadas de control de flujo: if/then/else, while/do,
    repeat/until, case/of, break, continue.
...
reservadas = {
    ...
    'var': 'VAR',
    'const': 'CONST',
    # Los cuatro nombres de tipo comparten el mismo token TIPO. A la
    # gramática no le interesa distinguir "integer" de "real" como
    # producciones distintas — le basta con saber "aquí va un nombre de
    # tipo", y el texto exacto (t.value) ya viene incluido gratis.
    'integer': 'TIPO',
    'real': 'TIPO',
    'boolean': 'TIPO',
    'string': 'TIPO',
...
# Relacionales. Como con `:=` y `:`, PLY prueba primero la regex más
# larga (`<=` antes que `<`, `>=` antes que `>`), así que MENORIGUAL y
# MAYORIGUAL no necesitan ir "antes" de MENOR/MAYOR en el archivo — el
# orden aquí es solo por legibilidad.
t_IGUAL = r'=='
t_DISTINTO = r'!='
t_MENORIGUAL = r'<='
t_MAYORIGUAL = r'>='
