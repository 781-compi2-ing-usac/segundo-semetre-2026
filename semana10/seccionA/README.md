...
El lenguaje soporta arrays de múltiples dimensiones (1D, 2D, 3D, etc.) con almacenamiento en el stack y acceso mediante cálculo de offsets en tiempo de compilación.
...
### 7.2 Nodos AST para Arrays

| Nodo | Atributos | Descripción |
|------|-----------|-------------|
| `ArrayNode` | `array` (lista de expresiones) | Representa un literal de array. Los elementos pueden ser `PrimitiveNode` o `ArrayNode` anidados |
| `VariableNode` | `name`, `access` (lista) | `access` contiene las expresiones de índice. Ejemplo: `arr[1][2]` → `access = [PrimitiveNode(1), PrimitiveNode(2)]` |
| `AssignmentNode` | `var_name`, `access`, `expression` | `access` contiene las expresiones de índice para asignación a elementos |

### 7.3 ArrayValidator (`AST/Structures/array_validator.py`)

La clase `ArrayValidator` se encarga de:

1. **Validar tipos homogéneos**: Todos los elementos deben ser del mismo tipo
2. **Validar dimensiones consistentes**: En arrays anidados, todas las sub-arrays deben tener la misma forma
3. **Aplanar elementos**: Convierte la estructura anidada en una lista plana para almacenamiento en el stack
4. **Calcular la forma (shape)**: Determina las dimensiones del array
...
# Output:
#   element_type: "int"
#   flat_elements: [1, 2, 3, 4, 5, 6]
#   shape: (2, 3)
#   dimensions: 2
```

### 7.4 Almacenamiento en el Stack

Los elementos del array se almacenan secuencialmente en el stack:
...
str x9, [x29, #-8]      // arr[0]
mov x10, #2
str x10, [x29, #-16]    // arr[1]
mov x11, #3
str x11, [x29, #-24]    // arr[2]
mov x12, #4
str x12, [x29, #-32]    // arr[3]
mov x13, #5
str x13, [x29, #-40]    // arr[4]
```

Cada elemento ocupa 8 bytes (alineación ARM64), independientemente de su tipo.

### 7.5 Cálculo de Offsets (Row-Major)

El acceso a arrays multidimensionales usa el algoritmo row-major recursivo:

**Fórmula general:**
```
offset([i, j, k]) = offset([i, j]) * d_k + k
offset([i, j])    = offset([i]) * d_j + j
offset([i])       = i
```

Donde `d_i` es el tamaño de la dimensión `i`.

**Ejemplo para `matrix[1][2]` con shape (2, 3):**
```
offset = 1 * 3 + 2 = 5
dirección = FP + base_offset - (5 * 8)
```

**Implementación recursiva en `_calculate_array_offset`:**
```python
def _calculate_array_offset(self, indices, shape):
    if len(indices) == 1:
        # Caso base: cargar el índice en un registro
        return index_reg

    # Recursión: offset_parcial * dimensión + índice_actual
    partial_offset = self._calculate_array_offset(indices[:-1], shape[:-1])
    last_index = self.dispatch(indices[-1])
    dim_value = shape[-1]

    # Calcular: partial * dim + last_index
    temp_mul = multiply(partial_offset, dim_value)
    temp_add = add(temp_mul, last_index)
    return temp_add
```

### 7.6 Métodos del ARMBuilder para Arrays

| Método | Descripción |
|--------|-------------|
| `build_array_load(rd, base_var, offset_reg, type)` | Carga un elemento del array: `ldr rd, [FP + base - offset*8]` |
| `build_array_store(rd, base_var, offset_reg, type)` | Almacena un elemento en el array: `str rd, [FP + base - offset*8]` |
| `build_load_immediate(rd, value)` | Carga un valor inmediato en un registro |
| `build_multiply(rd, rs1, rs2)` | Multiplicación de registros |
| `build_add(rd, rs1, rs2)` | Suma de registros |

### 7.7 Ejemplo Completo: Array 2D
...
**Cálculos de offset:**
- `matrix[0][0]`: offset = 0*3 + 0 = 0 → dirección FP-8
- `matrix[1][1]`: offset = 1*3 + 1 = 4 → dirección FP-40

### 7.8 Limitaciones Actuales

1. **Solo ARM64**: El soporte de arrays está implementado solo en `ARMBuilder`. `TACBuilder` (LLVM IR) aún no tiene soporte completo.
