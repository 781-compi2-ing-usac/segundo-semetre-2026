...
class ARMBuilder(Builder):
    def __init__(self):
        ...
        self.type_sizes = {"int": 4, "float": 8, "bool": 1}

    def new_temp(self) -> str:
        self.temp_count += 1
        temp_regs = ["x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17"]
        return temp_regs[(self.temp_count - 1) % len(temp_regs)]
    ...
    def build_multiply(self, rd: str, rs1: str, rs2: str):
        self.emit(f"    mul {rd}, {rs1}, {rs2}")

    def build_add(self, rd: str, rs1: str, rs2: str):
        self.emit(f"    add {rd}, {rs1}, {rs2}")

    def build_load_immediate(self, rd: str, value: int):
        self.emit(f"    mov {rd}, #{value}")

    def build_array_load(
        self, rd: str, base_var: str, offset_reg: str, type_name: str = "int"
    ):
        if type_name == "float":
            raise NotImplementedError(
                "Float no soportado en ARMBuilder (versión básica)"
        ...
        var_info = self.variables.get(base_var)
        if var_info:
            base_offset = var_info["offset"]
            temp_offset = self.new_temp()
            temp_scaled = self.new_temp()
            self.emit(f"    mov {temp_offset}, #{base_offset}")
            self.emit(f"    lsl {temp_scaled}, {offset_reg}, #3")
            self.emit(f"    sub {temp_offset}, {temp_offset}, {temp_scaled}")
            self.emit(f"    ldr {rd}, [{REGISTROS['FP']}, {temp_offset}]")
        else:
            self.emit(f"    ldr {rd}, [{REGISTROS['FP']}, {offset_reg}]")

    def build_array_store(
        self, rd: str, base_var: str, offset_reg: str, type_name: str = "int"
    ):
        if type_name == "float":
            raise NotImplementedError(
        ...
        if var_info:
            ...
            self.emit(f"    sub {temp_offset}, {temp_offset}, {temp_scaled}")
            if self._is_literal(rd):
                temp_val = self.new_temp()
                self.emit(f"    mov {temp_val}, #{rd}")
                self.emit(f"    str {temp_val}, [{REGISTROS['FP']}, {temp_offset}]")
            else:
                self.emit(f"    str {rd}, [{REGISTROS['FP']}, {temp_offset}]")
        else:
            if self._is_literal(rd):
                temp_val = self.new_temp()
                self.emit(f"    mov {temp_val}, #{rd}")
                self.emit(f"    str {temp_val}, [{REGISTROS['FP']}, {offset_reg}]")
            else:
                self.emit(f"    str {rd}, [{REGISTROS['FP']}, {offset_reg}]")
