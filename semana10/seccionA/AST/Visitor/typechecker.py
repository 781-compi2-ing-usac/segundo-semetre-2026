...
from AST.errors import TypeCheckError
...
class TypeChecker(Visitor):
    ...
    def visit_variable(self, node: VariableNode):
        ...
        for access in node.access:
            ...
            if index_type != "int":
                self.error(
                    f"Array index for '{node.name}' must be int, got {index_type}", node
                )
                return None
    ...
    def visit_declaration(self, node: DeclarationNode):
        ...
        if node.expression:
            expr_type = self.dispatch(node.expression)
            if isinstance(expr_type, ArrayType):
                ...
                self.error(
                    f"Type mismatch in declaration: {var_type} and {expr_type}", node
                )
    ...
    def visit_assignment(self, node: AssignmentNode):
        ...
        if declared_type is None:
            self.error(f"Undefined variable: {node.var_name}", node)
        ...
        if declared_type is not None and not compatible:
            self.error(
                f"Type mismatch in assignment: {target_type} and {expr_type}", node
            )
        return target_type
    ...
    def visit_print(self, node: PrintNode):
        ...
        if (
            ...
            self.error(f"Invalid type for print statement: {expr_type}", node)
        return expr_type

    def visit_if(self, node: IfNode):
        condition_type = self.dispatch(node.condition)
    ...
    def visit_function_call(self, node: FunctionCallNode):
        ...
        if func_info is None:
            self.error(f"Undefined function: {node.func_name}", node)
            return None
    ...
    def visit_param(self, node: ParamNode):
        base_type = self.dispatch(node.param_type)
        if node.array_dimensions:
            param_type = ArrayType(base_type, (None,) * len(node.array_dimensions))
        else:
    ...
    def visit_return(self, node: ReturnNode):
        if node.expression:
            return self.dispatch(node.expression)
        return "void"

    def visit_array(self, node: ArrayNode):
        element_types = [self.dispatch(element) for element in node.array]
        if not element_types:
            self.error("Array cannot be empty", node)
            return None
        ...
        if isinstance(first_type, ArrayType):
            ...
            if any(
                ...
                self.error("Matrix rows must have the same type and length", node)
            return ArrayType(
                first_type.element_type, (len(node.array),) + first_type.shape
            )
