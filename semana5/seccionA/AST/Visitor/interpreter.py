...
class Interpreter(Visitor):
    ...
    def visit_variable(self, node: VariableNode):
        ...
        if not isinstance(value, ArrayValue):
            raise ArrayAccessError(f"Variable '{node.name}' is not an array")

        indexes = [access.visit(self) for access in node.access]
    ...
    def visit_assignment(self, node: AssignmentNode):
        ...
        if not isinstance(array_value, ArrayValue):
            raise ArrayAccessError(f"Variable '{node.var_name}' is not an array")
    ...
    def visit_array(self, node: ArrayNode):
        return ArrayValue([element.visit(self) for element in node.array])
